...
1. 中英对照（5分钟）：
   ...
   Kubernetes API服务器的主要实现是kube-apiserver。kube-apiserver被设计为水平扩展——即通过部署更多实例来扩展。可以运行多个kube-apiserver实例并在这些实例之间平衡流量。
2. 核心词汇提取（5分钟）：
     expose → 暴露
     front end → 前端（入口）
     scale horizontally → 水平扩展
3. 底层映射思考（15分钟）：
     API Server是K8s的“前台”，所有操作（kubectl命令、控制器查询）都先经过它。这和之前部署的Nginx作为前端入口是一个模式。
     “水平扩展”意味着无状态，可以加多个实例。这和在C语言里写过的无状态函数（每次调用不依赖上次结果）是同一思想。
